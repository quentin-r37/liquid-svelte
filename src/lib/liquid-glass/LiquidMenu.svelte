...
>
	...
	<!--
		...
		The panel drives all three optics from the morph, so `variant` reaches it for
		one thing only: the shape of the veil, which the material owns now (see
		`.lg[data-variant]` in liquidGlass.css). It used to arrive here as an
		`is-frosted` class carrying the same three figures, panel-only.
	-->
	<LiquidGlass
